...
subroutine krome_driver(d, e, ge, u, v, w, &
      ...
      gamma, fh, dtoh, kphHI, kphHeI, &
      kphHeII, kdissH2I, photogamma)
  ...
  real*8::kphHI(in,jn,kn)
  real*8::kphHeI(in,jn,kn)
  real*8::kphHeII(in,jn,kn)
  ...
  do k = ks+1, ke+1
	...
	kphHI(i,j,k) = kphHI(i,j,k) * factor
        kphHeI(i,j,k) = kphHeI(i,j,k) * factor
        kphHeII(i,j,k) = kphHeII(i,j,k) * factor
        kdissH2I(i,j,k) = kdissH2I(i,j,k) * factor
        photogamma(i,j,k) = photogamma(i,j,k) * factor
        ...
        kphHI(i,j,k) = max(kphHI(i,j,k), krome_tiny)
        kphHeI(i,j,k) = max(kphHeI(i,j,k), krome_tiny)
        kphHeII(i,j,k) = max(kphHeII(i,j,k), krome_tiny)
        kdissH2I(i,j,k) = max(kdissH2I(i,j,k), krome_tiny)
        photogamma(i,j,k) = max(photogamma(i,j,k), krome_tiny)
  ...
  do k = ks+1, ke+1
	...
	krome_x(krome_idx_kphHj) = kphHI(i,j,k)
	krome_x(krome_idx_kphHej) = kphHeI(i,j,k)
	krome_x(krome_idx_kphHejj) = kphHeII(i,j,k)
	krome_x(krome_idx_kdissH2j) = kdissH2I(i,j,k)
	krome_x(krome_idx_photogamma) = photogamma(i,j,k)
	...
	kphHI(i,j,k) = krome_x(krome_idx_kphHj)
	kphHeI(i,j,k) = krome_x(krome_idx_kphHej)
	kphHeII(i,j,k) = krome_x(krome_idx_kphHejj)
	kdissH2I(i,j,k) = krome_x(krome_idx_kdissH2j)
	photogamma(i,j,k) = krome_x(krome_idx_photogamma)
